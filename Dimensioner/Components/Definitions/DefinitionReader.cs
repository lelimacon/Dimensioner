...
namespace Dimensioner.Components.Definitions
{
    public class DefinitionReader : TaxonomyComponentReader
    {
        ...
        private readonly ConcurrentDictionary<Definition, TmpLocator> _arcs;

        public DefinitionReader()
        {
            //_definitions = new Definitions();
            _arcs = new ConcurrentDictionary<Definition, TmpLocator>();
        }
        ...
        private Definition BuildDefinition(Linkbase linkbase, XElement node)
        {
            ...
            var definition = new Definition(schema, $"{schema.Namespace}:{role}");
